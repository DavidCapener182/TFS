...
export function BugTable() {
  ...
  const fetchStats = useCallback(async () => {
    ...
    const [openBugs, features, resolvedWeek, allResolved] = await Promise.all([
      supabase.from('user_feedback').select('id', { count: 'exact', head: true }).eq('type', 'bug').in('status', ['open', 'in_progress']),
      supabase.from('user_feedback').select('id', { count: 'exact', head: true }).eq('type', 'feature').in('status', ['open', 'in_progress']),
      supabase.from('user_feedback').select('id', { count: 'exact', head: true }).eq('status', 'resolved').gte('resolved_at', weekAgo),
      supabase.from('user_feedback').select('created_at, resolved_at').eq('status', 'resolved').not('resolved_at', 'is', null),
    ])
